...
def Visualize_ACM(model, signal, cfg, index):
    ...
    fig1.savefig(cfg.result_dir + '/' + f'visualize_before_ACM#{index}.svg', format='svg', dpi=600)
    ...
    fig2.savefig(cfg.result_dir + '/' + f'visualize_after_ACM#{index}.svg', format='svg', dpi=600)
    plt.close()
...
def Draw_Confmat(Confmat_Set, snrs, cfg):
    for i, snr in enumerate(snrs):
        ...
        fig.savefig(conf_mat_dir + '/' + f'ConfMat_{snr}dB.svg', format='svg', dpi=600)
        plt.close()
...
def Snr_Acc_Plot(Accuracy_list, Confmat_Set, snrs, cfg):
    ...
    fig2.savefig(acc_dir + '/' + 'acc.svg', format='svg', dpi=600)
    plt.close()
    ...
    plt.savefig(acc_dir + '/' + 'acc_mods.svg', format='svg', dpi=600)
    plt.close()
...
def save_training_process(train_process, cfg):
    ...
    plt.grid()
    fig1.savefig(cfg.result_dir + '/' + 'lr.svg', format='svg', dpi=600)
    plt.close()
    ...
    plt.legend()
    plt.grid()
    fig2.savefig(cfg.result_dir + '/' + 'loss_acc.svg', format='svg', dpi=600)
    plt.show()
    plt.close()
